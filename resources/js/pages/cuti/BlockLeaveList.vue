...
const form = ref({ tanggal_mulai: '', alasan: '' });
...
async function load() {
    loading.value = true;
    try {
        const { data } = await api.get('/block-leave', { params: { page: page.value } });
...
async function save() {
    saving.value = true;
    try {
        await api.post('/block-leave', form.value);
        toastOk('Pengajuan block leave berhasil.');
...
async function doApproval(status) {
    ...
    try {
        await api.post(`/block-leave/${showApproval.value.id}/${status}`, { catatan: catatan.value });
...
<template>
    ...
    <Modal v-if="showApproval" title="Review Block Leave" @close="showApproval = null">
